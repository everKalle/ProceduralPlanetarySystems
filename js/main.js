...
function generateStar(pivot, orbit, orbitPeriod, baseRotation, depth, minimalOrbit){
	...
	if (Math.random() <= 0.8 || depth == 0){
		...
		if (Math.random() <= 0.6){	//Close orbiting stars
			...
			star1 = addStar(binary, starMass1, binaryOrbitDistance, getOrbitalPeriod(starMass1, binaryOrbitDistance), binaryRot, 36000.0 + Math.random() * 15360000.0, minOrbit);
			star2 = addStar(binary, starMass2, binaryOrbitDistance, getOrbitalPeriod(starMass1, binaryOrbitDistance), binaryRot + 180, 36000.0 + Math.random() * 15360000.0, minOrbit);
...
function generatePlanets(pivot, baseMass, maxCount, baseDistance, distanceIncreaseBase, distanceIncreaseRandom, minimalOrbit, lightObject){
	...
	for(i = 0; i < numPlanets; i++){
		if (Math.random() <= 0.9){
			...
			var moonOrbitRadius = 16500;
			//minOrbit += planetRadius + 50;
			if (distance < CHZ_MidPoint + 39597871 && distance > CHZ_MidPoint - 39597871){
				minOrbit += planetRadius + 50;
				planet = addPlanet(pivot, planetRadius, distance * distanceMultiplier, getOrbitalPeriod(solarMass * baseMass, distance), 360.0 * Math.random(), 3600.0 + Math.random() * 1536000.0, 1, minOrbit, lightObject);
			} else if (distance > CHZ_MidPoint + 39597871) {
				if (Math.random() <= 0.4) {
					minOrbit += planetRadius + 50;
					planet = addPlanetOther(pivot, planetRadius, distance * distanceMultiplier, getOrbitalPeriod(solarMass * baseMass, distance), 360.0 * Math.random(), 3600.0 + Math.random() * 1536000.0, 1, minOrbit, lightObject);
				} else {
					minOrbit += gasGiantRadius + 50;
					moonOrbitRadius = gasGiantRadius * earthRadius + 16500;
					planet = addPlanetGasGiant(pivot, gasGiantRadius, distance * distanceMultiplier, getOrbitalPeriod(solarMass * baseMass, distance), 360.0 * Math.random(), 3600.0 + Math.random() * 1536000.0, 1, minOrbit, lightObject);
				}
			} else {
				minOrbit += planetRadius + 50;
				planet = addPlanetOther(pivot, planetRadius, distance * distanceMultiplier, getOrbitalPeriod(solarMass * baseMass, distance), 360.0 * Math.random(), 3600.0 + Math.random() * 1536000.0, 1, minOrbit, lightObject);
			...
			var planet2;
			if (distance < CHZ_MidPoint + 39597871 && distance > CHZ_MidPoint - 39597871){
				planet1 = addPlanet(binary, planetMass1, binaryOrbitDistance * distanceMultiplier, getOrbitalPeriod((planetMass1 + planetMass2) * earthMass, binaryOrbitDistance), binaryRot, 3600.0 + Math.random() * 1536000.0, 1, (planetMass1 + planetMass2 + 50) * distanceMultiplier, lightObject);
			} else {
				planet1 = addPlanetOther(binary, planetMass1, binaryOrbitDistance * distanceMultiplier, getOrbitalPeriod((planetMass1 + planetMass2) * earthMass, binaryOrbitDistance), binaryRot, 3600.0 + Math.random() * 1536000.0, 1, (planetMass1 + planetMass2 + 50) * distanceMultiplier, lightObject);
			}
			
			generateMoons(planet1, 16500, planetMass1*2, lightObject);
			
			if (distance < CHZ_MidPoint + 39597871 && distance > CHZ_MidPoint - 39597871){
				planet2 = addPlanet(binary, planetMass2, binaryOrbitDistance * distanceMultiplier, getOrbitalPeriod((planetMass1 + planetMass2) * earthMass, binaryOrbitDistance), binaryRot + 180, 3600.0 + Math.random() * 1536000.0, 1, (planetMass1 + planetMass2 + 50) * distanceMultiplier, lightObject);
			} else {
				planet2 = addPlanetOther(binary, planetMass2, binaryOrbitDistance * distanceMultiplier, getOrbitalPeriod((planetMass1 + planetMass2) * earthMass, binaryOrbitDistance), binaryRot + 180, 3600.0 + Math.random() * 1536000.0, 1, (planetMass1 + planetMass2 + 50) * distanceMultiplier, lightObject);
			}
			generateMoons(planet2, 16500 , planetMass1*2, lightObject);
		}
...
function generateMoons(pivot, baseOrbitDistance, minimalOrbit, lightObject){
	...
	for(j = 0; j < numMoons; j++){
		...
		if (Math.random() >= 0.3){
			...
			moon = addPlanetOther(pivot, 1 * Math.random(), Math.max(moonDistance * distanceMultiplier, 1.0), getOrbitalPeriod(earthMass, moonDistance), 360.0 * Math.random(), 3600.0 + Math.random() * 1536000.0, 2, minOrbit, lightObject);
		}
